...
def should_retry(exception: Exception):
    ...
    if isinstance(exception, ECSOperatorError):
        return any(
            ...
            for quota_reason in ['RESOURCE:MEMORY', 'RESOURCE:CPU', "AGENT"]
...
class ECSOperator(BaseOperator):
    ...
    def execute(self, context, session=None):
        ...
        if not self.do_xcom_push: return None
    ...
    @AwsBaseHook.retry(should_retry=should_retry)
    ...
    def _wait_for_task_ended(self) -> None:
        ...
        waiter.wait(cluster=self.cluster, tasks=[self.arn], WaiterConfig=self.waiter_config)
    ...
    def _check_success_task(self) -> None:
        ...
        for task in response['tasks']:
            ...
            for container in containers:
                if container.get('lastStatus') == 'STOPPED' and container.get('exitCode', 0) != 0:
                    last_logs = "\n".join(
